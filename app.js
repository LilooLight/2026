/* app.js — логика портфолио Марии Мишиной
   Словарь: RU / EN / CN — полный, со всеми ключами контента.
   Финальная версия после локализации: имена, единицы, контексты2, кольцевая навигация.
*/
(function(){
'use strict';
...
const I18N = {
  ru: {
    ...
    gnvName:'ГлавНИВЦ', sberName:'Сбер', otkName:'Банк «ФК Открытие»', conName:'IT-компании-подрядчики',
    ...
    kpiContext:'Компания — государственный интегратор и центр компетенций для высшего эшелона власти (УДП РФ). Продукт — внутренняя CRM для управления персоналом и МИС, работающая на устаревшем фреймворке Bootstrap с 200+ разрозненными экранами и отсутствием единой логики.',
    kpiContext2:'Требовалось улучшить UX «здесь и сейчас» и заложить основу для миграции на новый технологический стек.',
    kpiSolution:'Работа выстроена по двум параллельным трекам — быстрый результат для пользователей и системная база для будущего движка.',
    ...
    kpiPrev:'Предыдущий кейс: Бизнес-портал', kpiNext:'Следующий кейс: МИС «Альтера»',
    ...
    altContext:'Компания (в структуре ГлавНИВЦ) владела устаревшей МИС «Практика.Санаторий» на legacy-стеках. Бизнес-заказчик хотел новый современный модуль для врачей, но поставил жёсткие рамки: нет доступа к живой системе — только 10 статичных скриншотов, и нет возможности пообщаться с реальными врачами-пользователями.',
    altContext2:'Основная задача — создать эргономичный, минималистичный интерфейс для самой популярной роли, терапевта (работает с 13–17 пациентами в день), минимизировав когнитивную нагрузку и время на ввод данных.',
    ...
    altPrev:'Предыдущий кейс: Редизайн CRM «КПИ»', altNext:'Следующий кейс: СБОЛ.про',
    altFootnote:'Полные технические спецификации и стратегическое обоснование для разработки (Blazor, UI-паттерны) доступны', altFootnoteLink:'по запросу',
    ...
    sbContext:'Сотрудники офисов банка переходили на новую справочную систему, которая заменила несколько информационно-справочных систем. Старая система строилась на жёстком последовательном 5-уровневом классификаторе. Это приводило к высокой нагрузке на внутренних специалистов, частым ошибкам классификации и увеличению времени решения проблем клиентов.',
    sbContext2:'Критической задачей было обеспечить плавный переход без падения качества обслуживания.',
    sbRoleLbl:'Моя роль',
    sbRoleRest:'Проведение UX-исследований (гемба, глубинные интервью, UX-тестирование), анализ эффективности работы, проектирование AI-помощника, координация с командой разработки и data science.',
    ...
    ndaText:'Продукт находится под соглашением о неразглашении. Визуальные материалы не подлежат экспорту из внутреннего контура банка — поэтому вместо скриншотов экранов здесь представлены ключевые метрики, типографика и табличные данные. Полное описание процесса и архитектуры доступно по запросу.',
    sbPrev:'Предыдущий кейс: МИС «Альтера»', sbNext:'Следующий кейс: SmartCare',
    ...
    scPrev:'Предыдущий кейс: СБОЛ.про', scNext:'Следующий кейс: Бизнес-портал',
    ...
    bpRole1:'Senior (2016 — 2019) — единственный дизайнер проекта.', bpRole1Rest:'Спроектировала и запустила MVP с нуля, проводила первые фокус-группы и исследования, заложила основы архитектуры интерфейса.',
    bpRole2:'Lead Product Designer / Design Team Lead (2019 — 2022).', bpRole2Rest:'Выстроила дизайн-функцию с нуля, создала дизайн-систему на Atomic Design.',
    ...
    bpRiaLbl:'Публичное признание', bpRiaPre:'РИА Новости · 2020 ·', bpRiaCap:'открыть статью',
    ...
    bpBpNdaLbl:'Что не видно на скриншотах.', bpBpNda:'Проект закрыт в 2023 году в связи с поглощением ВТБ. Полные материалы доступны по запросу.',
  ...
  en: {
    ...
    gnvName:'GlavNIVC', sberName:'Sber', otkName:'Otkritie Bank', conName:'IT contractor companies',
    ...
    kpiContext:'The company is a government integrator and competence center for the highest echelon of power (Presidential Administration). The product is an internal CRM for HR management and an HIS, running on an outdated Bootstrap framework with 200+ scattered screens and no unified logic.',
    kpiContext2:'The goal was to improve UX “here and now” and lay the foundation for migration to a new technology stack.',
    ...
    kpiPrev:'Previous case study: Business Portal', kpiNext:'Next case study: Altera HIS',
    ...
    altContext:'The company (within GlavNIVC) owned an outdated HIS “Praktika.Sanatoriy” on legacy stacks. The business owner wanted a new modern module for physicians but set strict constraints: no access to the live system — only 10 static screenshots, and no opportunity to talk to real physician users.',
    altContext2:'The main goal was to create an ergonomic, minimalist interface for the most common role, the general practitioner (who sees 13–17 patients per day), minimizing cognitive load and data entry time.',
    ...
    altStage3:'Stage 03 · Defending the UX approach before development', altStage3Text:'Instead of using the off-the-shelf Blazor SDK “as is” — justification based on Hick’s and Miller’s laws. Proposed the “Skeleton + Skin” architecture: the SDK handles logic, while a custom design system (CSS variables and wrapper components) handles layout and ergonomics. This did not break the current development and eliminated duplicated work in the future.',
    ...
    altPrev:'Previous case study: CRM “KPI” Redesign', altNext:'Next case study: Sbol.pro',
    altFootnote:'Full technical specifications and strategic rationale for development (Blazor, UI patterns) are available', altFootnoteLink:'upon request',
    ...
    sbContext:'Bank branch employees were transitioning to a new reference system that replaced several information systems. The old system was built on a rigid sequential 5-level classifier. This led to high load on internal specialists, frequent classification errors, and increased time to resolve customer problems.',
    sbContext2:'The critical task was to ensure a smooth transition without a drop in service quality.',
    sbRoleLbl:'My role',
    sbRoleRest:'Conducted UX research (gemba, in-depth interviews, UX testing), analyzed work efficiency, designed the AI assistant, coordinated with the development and data science teams.',
    ...
    ndaText:'The product is under a non-disclosure agreement. Visual materials cannot be exported from the bank’s internal environment — therefore, instead of screenshots, key metrics, typography, and tabular data are presented here. A full description of the process and architecture is available upon request.',
    sbPrev:'Previous case study: Altera HIS', sbNext:'Next case study: SmartCare',
    ...
    scPrev:'Previous case study: Sbol.pro', scNext:'Next case study: Business Portal',
    ...
    bpRole1:'Senior (2016 — 2019) — the only designer on the project.', bpRole1Rest:'Designed and launched the MVP from scratch, conducted the first focus groups and research, laid the foundations of the interface architecture.',
    bpRole2:'Lead Product Designer / Design Team Lead (2019 — 2022).', bpRole2Rest:'Built the design function from scratch, created a design system based on Atomic Design.',
    ...
    bpRiaLbl:'Public recognition', bpRiaPre:'RIA Novosti · 2020 ·', bpRiaCap:'open article',
    ...
    bpBpNdaLbl:'What is not visible in the screenshots.', bpBpNda:'The project was closed in 2023 due to the VTB acquisition. Full materials are available upon request.',
  ...
  cn: {
    ...
    kpiContext:'公司是为最高权力层（俄罗斯总统办公厅）服务的政府集成商和咨询中心。产品是用于人事管理和医疗信息系统（HIS）的内部 CRM，基于陈旧的 Bootstrap 框架运行，拥有 200 多个零散界面，缺乏统一逻辑。',
    kpiContext2:'需要在“当下”改善 UX，并为迁移到新技术栈奠定基础。',
    ...
    kpiPrev:'上一个案例：Business Portal', kpiNext:'下一个案例：Altera 医疗信息系统',
    ...
    altContext:'公司（隶属 GlavNIVC）拥有基于陈旧技术栈的旧版医疗信息系统「Praktika.Sanatoriy」。业务方希望为医生打造全新的现代化模块，但设置了严格限制：无法访问真实系统——只有 10 张静态截图；也无法与真实医生用户交流。',
    altContext2:'核心任务是为最常用角色——全科医生（每天接诊 13–17 名患者）创建工效、极简的界面，最大程度降低认知负担和数据录入时间。',
    ...
    altPrev:'上一个案例：CRM「KPI」改版', altNext:'下一个案例：Sbol.pro',
    ...
    sbContext:'银行网点员工正在切换到新的查询系统，以取代多个信息查询系统。旧系统基于严格串行的 5 级分类器，导致内部专家负担重、分类错误频发、解决客户问题的时间延长。',
    sbContext2:'关键任务是确保平稳过渡，不降低服务质量。',
    sbRoleLbl:'我的角色',
    sbRoleRest:'开展 UX 研究（现场观察、深度访谈、UX 测试），分析工作效率，设计 AI 助手，与开发团队和数据科学团队协调。',
    ...
    ndaText:'产品受保密协议约束。视觉素材不得导出银行内网环境，因此此处以关键指标、排版和表格数据代替界面截图。完整流程和架构说明可应要求提供。',
    sbPrev:'上一个案例：Altera 医疗信息系统', sbNext:'下一个案例：SmartCare',
    scCardTitle:'SmartCare', scCardLbl:'满意度 · 两年演进',
    scMeta:'2023 — 2025 · 产品设计师 · NDA',
    scTitle:'SmartCare：支持工具的演进', scSub:'Sber 支持热线 · [ 2023年 — 2025年 ]',
    scContext:'SmartCare 是 Sber 支持热线员工的内部工具。问题不在于某个功能，而在于缺乏系统性：不同产品团队各自推出自己的模式和场景，造成混乱。这导致操作员认知负担高、电话处理时间延长、新人培训周期长。',
    scContext2full:'任务是在两年内平稳、系统性地提升支持工作效率，避免剧烈变动。',
    scRoleRest:'我的策略不是“一次性改版”，而是持续迭代的产品演进。大量 UX 研究不仅验证假设，还揭示了团队之间工作模式中的深层冲突。关键决策是标准化：将门户所有团队的场景和模式统一到同一标准。',
    ...
    scPrev:'上一个案例：Sbol.pro', scNext:'下一个案例：Business Portal',
